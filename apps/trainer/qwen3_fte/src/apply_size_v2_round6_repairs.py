#!/usr/bin/env python3
"""Apply manually confirmed round-6 paired-thickness fixes and siblings."""
...
DEFAULT_SAMPLE = Path("/private/tmp/v2_size_manual_sample_200_round6.json")
DEFAULT_REPORT = BASE_DIR / "16_人工抽查200条第六轮审核与修复结果.json"

AUDITED_ERRORS = {
    50397: "异径管第二端明示4.0mm漏标",
    41362: "异径三通支管明示SCH10S漏标",
    44133: "异径三通支管明示3.5mm漏标",
}
...
def main() -> int:
    ...
    for index, row in enumerate(repaired):
        ...
        if not isinstance(output, dict):
            continue
        if description_skeleton(text) not in target_skeletons:
            continue
    ...
    if missing:
        raise ValueError(f"第六轮错误未全部修复: {missing}")
    ...
    report = {
        "说明": "第六轮200条均逐条人工审核；确认错误及同骨架问题按原文明示的成对壁厚修复。",
