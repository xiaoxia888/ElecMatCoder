...
训练一个模型同时完成：
1. NER: 序列标注 (TYPE, MATERIAL, SIZE, THICKNESS, PRESSURE, STANDARD, CONN, MANU)
2. TYPE分类: 判断材料大类（管子/管件/法兰/螺栓/阀门/垫片）

使用方法:
    # 使用默认配置训练（从标注平台导出的JSONL）
    python -m apps.trainer.train_multitask --data_file data/pipe/multitask_data_2026-01-09.jsonl
    
    # 指定配置文件
    python -m apps.trainer.train_multitask --config src/bert_ner/config/training.yml
    
    # 命令行覆盖参数
    python -m apps.trainer.train_multitask --epochs 20 --batch_size 32
"""
...
from src.bert_ner.multitask_dataset import MultiTaskDataset
...
def plot_training_curves(history: dict, output_dir: str):
    ...
    ax.plot(epochs, [acc * 100 for acc in history['val_ner_acc']], 'g-o', label='NER', linewidth=2, markersize=6)
    ax.plot(epochs, [acc * 100 for acc in history['val_type_acc']], 'b-s', label='TYPE', linewidth=2, markersize=6)
    
    avg_acc = [(n + t) / 2 * 100 for n, t in zip(history['val_ner_acc'], history['val_type_acc'])]
    ax.plot(epochs, avg_acc, 'k-D', label='Average', linewidth=2.5, markersize=5)
    
    if history['best_epoch'] > 0:
        best_idx = history['best_epoch'] - 1
        ax.scatter([history['best_epoch']], [avg_acc[best_idx]], color='orange', s=200, zorder=5, marker='*', label=f'Best: {avg_acc[best_idx]:.1f}%')
...
def print_training_summary(history: dict, test_metrics: dict, output_dir: str, config: dict):
    ...
    summary.append(f"  准确率曲线: {os.path.join(output_dir, 'accuracy_curves.png')}")
    summary.append("=" * 70)
...
def train_epoch(model, dataloader, optimizer, scheduler, device, epoch, 
    ...
    Args:
        ...
        device: 设备
        epoch: 当前轮次
        dice_loss_fn: Dice Loss 函数（可选）
        dice_weight: Dice Loss 权重
        accumulation_steps: 梯度累积步数（默认1，即不累积）
    """
    model.train()
    ...
    for step, batch in enumerate(progress_bar):
        ...
        total_loss += loss.item() * accumulation_steps  # 还原实际 loss 值
...
def extract_entities(labels: List[int], id2label: dict) -> set:
    """
    从标签序列中提取实体（用于计算实体级 F1）
    
    Returns:
        set of (entity_type, start, end) tuples
    """
    entities = set()
    current_type = None
    start = None
...
def evaluate(model, dataloader, device, id2label: dict = None) -> dict:
    ...
    Args:
        ...
        dataloader: 数据加载器
        device: 设备
        id2label: 标签ID到标签名的映射（用于实体级F1计算）
    """
    model.eval()
...
def main():
    parser = argparse.ArgumentParser(description='训练多任务BERT模型')
    parser.add_argument('--config', type=str, default='src/bert_ner/config/training.yml',
                        help='配置文件路径')
    # 数据来源（二选一）
    parser.add_argument('--data_file', type=str, help='标注平台导出的JSONL文件（自动划分train/dev/test）')
    parser.add_argument('--data_dir', type=str, help='已划分好的数据目录（包含train.jsonl, dev.jsonl, test.jsonl）')
    # 其他参数
    ...
    logger.info(f"TYPE分类: {list(type_label2id.keys())}")
    ...
    # 加载tokenizer（自动适配不同模型）
    bert_model = config['model']['bert_model']
    logger.info(f"加载tokenizer: {bert_model}")
    tokenizer = AutoTokenizer.from_pretrained(bert_model)
    
    # 添加 [SPACE] 特殊 token（用于替代空格，让模型能"看到"空格）
    special_tokens = {'additional_special_tokens': ['[SPACE]']}
    num_added = tokenizer.add_special_tokens(special_tokens)
    logger.info(f"添加特殊token [SPACE]，新增 {num_added} 个token，词表大小: {len(tokenizer)}")
    
    # 加载数据集
    max_length = config['data']['max_length']
    augment_enabled = config['data'].get('augment', True)  # 默认启用数据增强
    ...
    train_dataset = MultiTaskDataset(
        ...
        augment=augment_enabled,  # 训练集启用数据增强
    ...
    # 调整模型的 embedding 大小以适应新增的 [SPACE] token
    model.encoder.resize_token_embeddings(len(tokenizer))
    logger.info(f"调整 embedding 大小为: {len(tokenizer)}")
    ...
    # 差分学习率：BERT 层使用较小学习率，CRF 和任务头使用较大学习率
    bert_params = []
    crf_params = []
    classifier_params = []
    ...
    optimizer = AdamW([
        ...
        {'params': crf_params, 'lr': crf_lr, 'weight_decay': 0.0},  # CRF 不用 weight_decay
        {'params': classifier_params, 'lr': classifier_lr, 'weight_decay': weight_decay}
    ])
    
    logger.info(f"差分学习率: BERT={lr:.2e}, CRF={crf_lr:.2e} (x{crf_lr_mult:.0f}), Classifier={classifier_lr:.2e} (x{classifier_lr_mult:.0f})")
    ...
    if accumulation_steps > 1:
        logger.info(f"启用梯度累积: {accumulation_steps} 步 (等效 batch_size={batch_size * accumulation_steps})")
    ...
    for epoch in range(1, epochs + 1):
        ...
        log_msg = f"  NER: {train_metrics['ner_loss']:.4f}, TYPE: {train_metrics['type_loss']:.4f}"
        if 'dice_loss' in train_metrics:
            log_msg += f", Dice: {train_metrics['dice_loss']:.4f}"
        logger.info(log_msg)
        ...
        logger.info(f"  NER 实体级 P/R/F1: {dev_metrics['ner_precision']*100:.2f}% / {dev_metrics['ner_recall']*100:.2f}% / {dev_metrics['ner_f1']*100:.2f}%")
        ...
        # 使用实体级 F1 作为主要指标（更能反映真实性能）
        avg_acc = dev_metrics['ner_f1'] * 0.7 + dev_metrics['type_accuracy'] * 0.3
        logger.info(f"  综合指标: {avg_acc*100:.2f}% (NER F1权重70%, TYPE准确率30%)")
    ...
    logger.info(f"测试集 NER P/R: {test_metrics['ner_precision']*100:.2f}% / {test_metrics['ner_recall']*100:.2f}%")
