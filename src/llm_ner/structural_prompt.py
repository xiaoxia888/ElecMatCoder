...
3. **壁厚**: 提取壁厚规格
   ...
   - **🔒 尺寸上下文排他（最高硬约束）**：若某个数值已经在已识别尺寸上下文中被判为 `DN/OD/INCH`，则该同值证据不得再次输出为壁厚，除非原文中有明确且直接绑定到该值的壁厚锚点（如 `THK=`、`T=`、`壁厚`、`厚度=`、或明确的 `外径×厚度` / `尺寸×厚度` 结构）。也就是说，“已识别为尺寸”的证据默认具有排他性，不能因为它同时是英寸值或裸数字，就再次改判成壁厚。
   - **🔒 英寸值归属规则（最高硬约束）**：英寸值默认优先归入尺寸语义，而不是壁厚语义。只有当英寸值被明确壁厚锚点直接约束时，才允许输出为 `INCH` 壁厚；若英寸值只是口径/NPS/规格的一部分，或已在尺寸上下文中出现，即使当前壁厚为空，也不得把该英寸值补成壁厚。
   - **壁厚判定顺序（硬约束）**：
   ...
   - **S 系等级壁厚公共规则（硬约束）**：`S-10S`、`10S`、`S10S`、`SCH10S` 本质上都是合法等级壁厚写法；若它们由 `x/X/×/*` 连接形成并列表达（如 `S-10S x S-10S`、`10S×10S`、`SCH10S*SCH10S`），必须按原文顺序输出多个 `SCHEDULE` item，不得返回空，也不得把其中任何一段改判成尺寸、压力或材质。
...
5. **磅级**: 提取压力等级信息
   ...
   - **禁止裸数字直接输出**：不得输出裸数字 `150`、`300`、`16`、`25` 作为 `pressure_rating`。当数字本身像压力后缀时，必须结合描述整体上下文先判断其所属体系，再输出完整结果：若上下文指向 PN/MPa 公制体系，则输出 `PN数字`；若上下文指向 CL/Class/LB/# 美标体系，则输出 `C数字`、`CL数字`、`Class 数字` 或原文 `数字#` 体系值。若仅看到一个可疑数字但无法从上下文稳定判断是 PN 还是 CL，则输出空字符串，不得猜测。
   - **禁止裸数字脑补磅级**：不得仅凭尺寸/型号/标准号/壁厚/材质中的数字推断 `CL/C/Class`，如 `DN150`、`BL150-16`、`Buff#300`、`Φ168.3` 均不能作为 `CL150/C150/CL300/C300` 证据；只有原文明确出现 `CL/CLASS/C/LB/LBS/数字#`，或该数字处在明确的法兰短型号压力后缀上下文中时，才可输出压力等级。
   ...
   - **法兰短型号公共规则（硬约束）**：当描述中出现“法兰短型号块”时，即存在“法兰缩写/法兰局部型号 + 规格 + 可选括号类型 + `-` + 数字后缀”的结构，该后缀数字优先解释为压力后缀，而不是第二尺寸。这个规则不依赖具体缩写必须是 `WN/SO/BL/PL`，也适用于 `SW` 等其他法兰局部型号写法；同时不要求压力后缀右侧必须紧贴 `RF/FF/RTJ`，只要在同一局部块中后续仍能看到 `RF/FF/RTJ`，即使中间夹有壁厚、材质或其他局部属性，该后缀仍应视为压力后缀。对这类后缀数字，必须结合整体上下文判体系后再输出：若上下文更像美标法兰体系（如 `ASME B16.5`、`Class/CL/LB/#` 语境），输出 `C数字`；若上下文更像公制 PN 体系（如 `HG/T 20592`、`HG/T 20615`、`NB/T 47008`、`PN`/`MPa` 语境），输出 `PN数字`。不得把该后缀原样输出为裸数字。
...
- `S-10S x S-10S` → `THICKNESS_ITEMS=[{"type":"SCHEDULE","value":"S10S"},{"type":"SCHEDULE","value":"S10S"}]`
""".strip()
...
- `突面钢制管法兰盖:80-150 RF,16Mn,NB/T47008 HG/T20615-2009` → `PRESSURE="PN150"`
- `WN 80-150 RF, A105, ASME B16.5` → `PRESSURE="C150"`
- `Class I` → `PRESSURE=""`
""".strip()
