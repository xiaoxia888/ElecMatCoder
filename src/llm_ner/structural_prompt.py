...
尺寸规则：
- `DN100`、`DN 100`、`DN-100` -> `{"type":"DN","value":"DN100"}`。
...
- `φ219.1`、`Φ219.1`、`OD219.1`、`D219.1` -> `OD=219.1`。
...
边界示例：
- `对焊管接台 ... φ21.3XSCH80 DN125X15` 中，`SIZE_ITEMS` 应包含 `DN125`、`DN15`、`OD21.3`。
- `管子 ... φ60.3XSCH40 DN50` 中，`SIZE_ITEMS` 应包含 `OD60.3` 与 `DN50`，不要把 `60.3` 改判成 `DN`。
- `氟塑料衬里管 48x2.8 PN16 L=1000 ...` 中，`48` 更像 `OD`，`2.8` 更像 `MM`，因此 `SIZE_ITEMS` 只保留 `OD=48`，`LENGTH=1000`，不得把 `2.8` 留在尺寸里，不得补 `DN40`。
- `GB/T 14976;SH/T 3408 SMLSTee 33.4x2.77BW S30408` 中，`33.4` 应输出为 `OD`，`2.77` 应输出为 `MM`，不得再补出 `DN33.4` 或 `DN15`。
- `... CL600 ... 26.7x3.91 ...` 中，`600` 只属于压力，不得进入尺寸。
- `无缝不锈钢管 ... THK=2.5m S30403 ... DN15` 中，`DN15` 是显式尺寸，必须输出 `DN15`，不得因为同句存在 `THK`、规范号或材质而漏掉。
"""
...
- 规格若为 `3xDN65`，拆成 `INCH` 与 `DN` 两个 item，顺序与原文一致
- 规格若为 `DN65x3`，拆成 `DN` 与 `INCH` 两个 item，顺序与原文一致
...
- `THICKNESS_ITEMS` 中每个 item 都必须是**单个原子值**，严格按原文出现顺序输出；禁止把 `7.9MMX10MM`、`12MMXSCH40S`、`SCH40XSCH80` 这类复合结果塞进单个 item 的 `value`
- 如果最终壁厚结果是毫米壁厚，按单个毫米值逐项输出，例如 `7.9MMX10MM` 必须拆成 `{"type":"MM","value":"7.9MM"}` 与 `{"type":"MM","value":"10MM"}`
- 如果最终壁厚结果是毫米 + 等级混合壁厚，按原文顺序拆成多个原子 item，例如 `12MMXSCH40S` 必须拆成 `{"type":"MM","value":"12MM"}` 与 `{"type":"SCHEDULE","value":"SCH40S"}`
...
- `3"*DN65` → `SIZE_ITEMS=[{"type":"INCH","value":"3"},{"type":"DN","value":"DN65"}]`, `LENGTH=""`
- `2 in` → `SIZE_ITEMS=[{"type":"INCH","value":"2"}]`, `LENGTH=""`
- `8 x 6 in` → `SIZE_ITEMS=[{"type":"INCH","value":"8"},{"type":"INCH","value":"6"}]`, `LENGTH=""`
- `DN200, L=3m` → `SIZE_ITEMS=[{"type":"DN","value":"DN200"}]`, `LENGTH="3000"`
""".strip()
...
- `Φ508X7.9/Φ325X10` → `THICKNESS_ITEMS=[{"type":"MM","value":"7.9MM"},{"type":"MM","value":"10MM"}]`
- `12mm*SCH40S` → `THICKNESS_ITEMS=[{"type":"MM","value":"12MM"},{"type":"SCHEDULE","value":"SCH40S"}]`
